...
function removeDuplicateLetters(s: string): string {
    const lastIndex: { [key: string]: number }= {}; // Stores the last index of each character
    const inStack: { [key: string]: boolean }= {};  // Keeps track of characters in the stack
    const stack: string[]= [];                      // Stack to build the result string
    ...
    for (let i= 0; i< s.length; i++) {
        ...
        if (inStack[char]) continue;  // Skip if already in stack
