...
function beautifulPartitions(s, k, minlength) {
    const MOD = 1000000007; // we use this keep our ans within a safe range
    const n = s.length;
    ...
    const canCut = new Array(n).fill(false);
    ...
    const memo = Array.from({ length: n }, () => Array(k + 1).fill(-1)); // To save answers
    function count(pos, parts) {
        if (parts === k) {
